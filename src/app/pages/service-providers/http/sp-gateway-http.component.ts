...
@Component({
    selector: 'app-service-providers-http',
    templateUrl: './sp-gateway-http.component.html',
})
export class HttpServiceProvidersComponent implements OnInit, OnDestroy{
    ...
    ngOnInit() {
        this.loadDtOptions();
        this.loadProviders();
        this.formModalDelete = new window.bootstrap.Modal(document.getElementById('modalDeleteHTTPServiceProvider'),)
        this.formModalBalance = new window.bootstrap.Modal(document.getElementById('modalBalanceHTTPServiceProvider'),)
    }

    async loadProviders() {
        this.response = await this.serviceProvidersService.getProviders();
        if (this.response.status == 200) {
            this.providers = this.response.data.filter((sp: any) => sp.protocol === 'HTTP')
